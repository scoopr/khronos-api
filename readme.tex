...
\begin{center}
{\tt https://cvs.khronos.org/svn/repos/ogl/trunk/doc/registry/public/api/}
\end{center}
...
\begin{itemize}
\item \tag{types} - defines API types. Usually only one tag is used.
\item \tag{enums} - defines API enumerants (tokens). Usually multiple
      tags are used.
\item \tag{commands} - defines API commands (functions). Usually only
      one tag is used.
\item \tag{feature} - defines API feature interfaces (API versions, more
      or less). One tag per feature set.
\item \tag{extensions} - defines API extension interfaces. Usually
      only one tag is used, wrapping many extensions.
\end{itemize}
...
\item \attr{class} - a string for grouping this set of enums together in
      the broader namespace, currently unused. Roughly corresponds to
      some of the functional \code{enum:} descriptions, such as
      \code{PixelFormat}, in the old \code{enum.spec} files.
...
Each \tag{enums} block contains zero of more \tag{enum} and \tag{unused}
...
\item \attr{alias} - name of another enumerant this is an alias
      (semantic equivalent) of, used where token names have been changed
      as a result of profile changes or for consistency purposes.
\end{itemize}
...
\item \tag{alias} has no attributes and contains a string which is the
      name of another command this command is a semantic alias of,
      typically used when promoting a function from extension to ARB or
      ARB to API status.
...
Some information from the \code{.spec} files is not included because it is
inaccurate (\code{version} tags from \code{.spec} rarely had anything to do
with reality, nor was their actual meaning well-defined) or cannot be easily
represented (such as the (supposed) length of array parameters). We will add
some of this information back in, probably via new attributes in the schema,
as the need becomes apparent and the information can be validated.

It would be a big job to go backwards from the XML to \code{.spec} formats,
and we don't want to support this or enhance the \code{.spec} files going
forward. Hopefully, people using the \code{.spec} files for other purposes
will be able to transition to the XML registry.
